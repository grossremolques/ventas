class Geo {
  ...
  static async getDepartamentoByProvincia(Provincia) {
    try {
      let response = await fetch(`https://apis.datos.gob.ar/georef/api/departamentos?provincia=${Provincia}&max=1000`);
      response = await response.json();
  ...
  static async getLocalidadByProvincia(Provincia, Departamento) {
    try {
      let response = await fetch(`https://apis.datos.gob.ar/georef/api/localidades-censales?provincia=${Provincia}&departamento=${Departamento}&aplanar=true&campos=estandar&max=100&exacto=true
      `);
      response = await response.json();
